...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 17 09:55:32 2018
...
#################################################################################################
    ...
    def train(self, indices):
        ...
        for vertex in self.vertices[indices[0]:indices[1]]:
            theta = self.forward_backward(vertex,0.002, 112.0,0.4)
        print("###########################It Ended ####################################")
        return self.THETA
...
data[data == 0] = -1
ising_model = Graph(data[1200:1800,:])
results = ising_model.train([0,256])
np.savetxt("model_ups_2.csv", results, delimiter=",")
