...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 17 00:38:30 2018
...
class Graph:
    ...
    def minimize_loss_theta(self,v, step):
        ...
        for i in range(150000):
            ...
            grad_sum = np.fabs(self.grad_node1 - grad_r - v.get_edges()[0]*grad_rt_2 - v.get_edges()[1]*grad_rt_3 + self.grad_node2 + self.grad_node3)
            ...
            if num_neighbors >=3:
                ...
                grad_sum += np.fabs(self.grad_node4 - v.get_edges()[2]*grad_rt_4)
                self.grad_node4 = v.get_edges()[2]*grad_rt_4

            if num_neighbors ==4:
                self.THETA[v.vertex_id,v.get_neighbors()[3]] -= v.get_edges()[3]*step*grad_rt_5
                grad_sum += np.fabs(self.grad_node5 - v.get_edges()[3]*grad_rt_5)
                self.grad_node5 = v.get_edges()[3]*grad_rt_5

            if grad_sum < 10**-4:
                break
            self.grad_node1 = grad_r
            self.grad_node2 = v.get_edges()[0]*grad_rt_2
            self.grad_node3 = v.get_edges()[1]*grad_rt_3
...
#################################################################################################
    def minimize_loss_edge(self,v,nb_vert_id,indx,step,mode):
        ...
        for i in range(150000):
            ...
            if np.fabs(grad_1 - self.grad_edge1) + np.fabs(grad_2 -self.grad_edge2)  < 10**-4:
                break
            self.grad_edge1 = grad_1
            self.grad_edge2 = grad_2
        ...
        while True:
            ...
            while True:
                ...
                if (delta_b > nu*delta_f) or (v.set_edges_length() == 0):
                    break
                remove_edge_to_vert = edges_r[min_loss_indx]
    ...
    def train(self, indices):
        ...
        for vertex in self.vertices[indices[0]:indices[1]]:
            theta = self.forward_backward(vertex,0.003, 97.0,0.4)
        print("###########################It Ended ####################################")
        return self.THETA
...
#results = ising_model.train([0,256])
#np.savetxt("model_ups_0.csv", results, delimiter=",")

file_path = "model_ups_1.csv"
theta = np.genfromtxt(file_path, dtype=float, delimiter=',') 

#print theta.shape
#for i in range(50):
#    for j in range(50):
#        print theta[i,j], theta[j,i], "i", i , "j", j

non_zero = np.where(theta != 0)
print non_zero
