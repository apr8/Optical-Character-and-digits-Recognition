...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 17 09:59:32 2018
...
#################################################################################################
    ...
    def train(self, indices):
        ...
        for vertex in self.vertices[indices[0]:indices[1]]:
            theta = self.forward_backward(vertex,0.002, 100.0,0.4)
        print("###########################It Ended ####################################")
        return self.THETA
...
data[data == 0] = -1
ising_model = Graph(data[3000:3600,:])
results = ising_model.train([0,256])
np.savetxt("model_ups_5.csv", results, delimiter=",")
