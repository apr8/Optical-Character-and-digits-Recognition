...
#################################################################################################
    def minimize_loss_edge(self,v,nb_vert_id,indx,step,mode):
        ...
        for i in range(150000):
            ...
            #print np.fabs(grad_1 - self.grad_edge1) + np.fabs(grad_2 -self.grad_edge2)
...
#        interim_exp = self.minimize_loss_theta(self.vertices[0],0.01)
        for vertex in self.vertices[indices[0]:indices[1]]:
            theta = self.forward_backward(vertex,0.025, 97.0,0.4)
        print("###########################It Ended####################################")
        return self.THETA
...
#results = Pool(1).map(ising_model.train, [[0,64], [64,128],[128,192], [192,256]])
results = ising_model.train([0,256])

np.savetxt("model_ups_1.csv", results, delimiter=",")
