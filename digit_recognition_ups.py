...
#            print(loss_edges)
            ...
            print("Active Edges Forward vert:  ", v.vertex_id, v.get_edges())
            v.expand_set_edges()
            
            loss_prev = self.loss_edges(v,theta_prev[min_loss_indx])
            delta_f = loss_prev - loss_edges[min_loss_indx]
            print("vert: ",v.vertex_id ,"delta F ", delta_f, "epsilon ", epsilon)
            if delta_f <= epsilon:
                break
            ...
            while True:
                ...
                print("vert: ", v.vertex_id,"delta B ", delta_b, "nu * DF ", nu*delta_f)
                ...
                print("Active Edges Backward vert: ", v.vertex_id, v.get_edges())
                v.reduce_set_edges()
...
#        interim_exp = self.minimize_loss_theta(self.vertices[0],0.01)
        for vertex in self.vertices[indices[0]:indices[1]]:
            theta = self.forward_backward(vertex,0.002,140.0,1e-4)
        print("###########################It Ended####################################")
        return self.THETA
...
results = Pool(7).map(ising_model.train, [[0,36], [36,72],[72,108], [108,144], [144,180], [180,216],[216,256]])

np.savetxt("model_ups_21.csv", results[0], delimiter=",")
np.savetxt("model_ups_22.csv", results[1], delimiter=",")
np.savetxt("model_ups_23.csv", results[2], delimiter=",")
np.savetxt("model_ups_24.csv", results[3], delimiter=",")
np.savetxt("model_ups_25.csv", results[4], delimiter=",")
np.savetxt("model_ups_26.csv", results[5], delimiter=",")
np.savetxt("model_ups_27.csv", results[6], delimiter=",")
