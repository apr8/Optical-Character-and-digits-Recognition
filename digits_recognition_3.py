...
# -*- coding: utf-8 -*-
"""
Created on Wed Apr 11 00:03:38 2018
...
# -*- coding: utf-8 -*-
"""
Created on Tue Mar 20 16:41:24 2018
...
#        self.EDGE = np.zeros((self.dataShape[2],self.dataShape[2]))
        ...
        self.leftNodes = []
        for i in range(33,1025,32):
            self.leftNodes.append(i)
        self.rightNodes = []
        for i in range(64,1056,32):
            self.rightNodes.append(i)
        self.bottomNodes = []
        for i in range(994,1024):
        ...
        for vert in self.vertices:
            if vert.vertex_id == 1:
                vert.add_neighbor([2,33])
                vert.add_edges([0,0])
            elif vert.vertex_id == 32:
                vert.add_neighbor([31,64])
                vert.add_edges([0,0])
            elif vert.vertex_id == 993:
                vert.add_neighbor([961,994])
                vert.add_edges([0,0])
            elif vert.vertex_id == 1024:
                vert.add_neighbor([992,1023])
                vert.add_edges([0,0])
            elif vert.vertex_id in self.topNodes:
                vert.add_neighbor([vert.vertex_id-1, vert.vertex_id+1, vert.vertex_id+32])
                vert.add_edges([0,0,0])
            elif vert.vertex_id in self.leftNodes:
                vert.add_neighbor([vert.vertex_id-32, vert.vertex_id+1, vert.vertex_id+32])
                vert.add_edges([0,0,0])
            elif vert.vertex_id in self.rightNodes:
                vert.add_neighbor([vert.vertex_id-32, vert.vertex_id-1, vert.vertex_id+32])
                vert.add_edges([0,0,0])
            elif vert.vertex_id in self.bottomNodes:
                vert.add_neighbor([vert.vertex_id-32, vert.vertex_id-1, vert.vertex_id+1])
                vert.add_edges([0,0,0])
            else:
    ...
    def loss_theta(self, v):
        ...
        for n_vert_id in v.get_neighbors():
            interim_sum += self.THETA[v.vertex_id-1,n_vert_id-1]*np.multiply(self.reshaped_data[:,v.vertex_id], self.reshaped_data[:,n_vert_id-1]).reshape((-1,1))
        interim_sum_ = interim_sum + self.THETA[v.vertex_id-1,v.vertex_id-1]*self.reshaped_data[:,v.vertex_id-1].reshape((-1,1))
        interim_exp = np.exp(interim_sum_)
        interim_log = np.log(np.add(1,interim_exp))
        interim_loss = np.subtract(interim_log, self.THETA[v.vertex_id-1,v.vertex_id-1]*self.reshaped_data[:,v.vertex_id-1])
    ...
    def loss_edges(self, v, theta):
        ...
        for indx, n_vert_id in enumerate(v.get_neighbors()):
            interim_sum += theta[0,indx+1]*np.multiply(self.reshaped_data[:,v.vertex_id-1], self.reshaped_data[:,n_vert_id-1]).reshape((-1,1))
        interim_sum_ = interim_sum + theta[0,0]*self.reshaped_data[:,v.vertex_id-1].reshape((-1,1))
        interim_exp = np.exp(interim_sum_)
        interim_log = np.log(np.add(1,interim_exp))
        interim_loss = np.subtract(interim_log, theta[0,0]*self.reshaped_data[:,v.vertex_id-1])
        interim_loss = np.subtract(interim_loss,interim_sum)
        real_loss = np.divide(np.sum(interim_loss), self.dataShape[2])
    ...
    def minimize_loss_theta(self,v, step):
        ...
        for i in range(150000):
            interim_sum = np.zeros((self.dataShape[2],1))
            for n_vert_id in v.get_neighbors():
                interim_sum += self.THETA[v.vertex_id-1,n_vert_id-1]*np.multiply(self.reshaped_data[:,v.vertex_id-1], self.reshaped_data[:,n_vert_id-1]).reshape((-1,1))
            interim_sum += self.THETA[v.vertex_id-1,v.vertex_id-1]*self.reshaped_data[:,v.vertex_id-1].reshape((-1,1))
            interim_exp = np.exp(interim_sum)
            num_1 = np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)), interim_exp)
            den = np.add(1,interim_exp)
            ratio_1 = np.divide(num_1,den)
            adj_ratio_1 = np.subtract(ratio_1,self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)))
            grad_r = np.divide(np.sum(adj_ratio_1),self.dataShape[2])

            
            num_2 = np.multiply(np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)),self.reshaped_data[:,v.get_neighbors()[0]-1].reshape((-1,1))),interim_exp)
            ratio_2 = np.divide(num_2,den)
            adj_ratio_2 = np.subtract(ratio_2,np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)),self.reshaped_data[:,v.get_neighbors()[0]-1].reshape((-1,1))))
            grad_rt_2 = np.divide(np.sum(adj_ratio_2),self.dataShape[2])
            
            num_3 = np.multiply(np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)),self.reshaped_data[:,v.get_neighbors()[1]-1].reshape((-1,1))),interim_exp)
            ratio_3 = np.divide(num_3,den)
            adj_ratio_3 = np.subtract(ratio_3,np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)),self.reshaped_data[:,v.get_neighbors()[1]-1].reshape((-1,1))))
            grad_rt_3 = np.divide(np.sum(adj_ratio_3),self.dataShape[2])
            
            if num_neighbors >= 3:
                num_4 = np.multiply(np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)),self.reshaped_data[:,v.get_neighbors()[2]-1].reshape((-1,1))),interim_exp)
                ratio_4 = np.divide(num_4,den)
                adj_ratio_4 = np.subtract(ratio_4,np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)),self.reshaped_data[:,v.get_neighbors()[2]-1].reshape((-1,1))))
                grad_rt_4 = np.divide(np.sum(adj_ratio_4),self.dataShape[2])
            
            if num_neighbors == 4:
                num_5 = np.multiply(np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)),self.reshaped_data[:,v.get_neighbors()[3]-1].reshape((-1,1))),interim_exp)
                ratio_5 = np.divide(num_5,den)
                adj_ratio_5 = np.subtract(ratio_5,np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)),self.reshaped_data[:,v.get_neighbors()[3]-1].reshape((-1,1))))
                grad_rt_5 = np.divide(np.sum(adj_ratio_5),self.dataShape[2])
                
            self.THETA[v.vertex_id-1,v.vertex_id-1] -= step*grad_r
            self.THETA[v.vertex_id-1,v.get_neighbors()[0]-1] -= v.get_edges()[0]*step*grad_rt_2
            self.THETA[v.vertex_id-1,v.get_neighbors()[1]-1] -= v.get_edges()[1]*step*grad_rt_3
            if num_neighbors >=3:
                self.THETA[v.vertex_id-1,v.get_neighbors()[2]-1] -= v.get_edges()[2]*step*grad_rt_4
            if num_neighbors ==4:
                self.THETA[v.vertex_id-1,v.get_neighbors()[3]-1] -= v.get_edges()[3]*step*grad_rt_5

##################################################################################################
# These are just a couple of print statements for debbuging purposes            
        print "For Vertex ", v.vertex_id
        print self.THETA[v.vertex_id-1,v.vertex_id-1], self.THETA[v.vertex_id-1,v.get_neighbors()[0]-1], self.THETA[v.vertex_id-1,v.get_neighbors()[1]-1]
        if num_neighbors >= 3:
            print self.THETA[v.vertex_id-1,v.get_neighbors()[2]-1]
        if num_neighbors ==4:
            print self.THETA[v.vertex_id-1,v.get_neighbors()[3]-1]
...
#################################################################################################    
    def minimize_loss_edge(self,v,nb_vert_id,indx,step,mode):
        ...
        interim_theta[0,0] = self.THETA_previous[v.vertex_id-1,v.vertex_id-1]
        num_neighbor = len(v.get_edges())
        for i in range(1,num_neighbor+1):
            interim_theta[0,i] = self.THETA_previous[v.vertex_id-1,v.get_neighbors()[i-1]-1]
        if mode == 1:
        ...
        for i in range(150000):
            interim_sum = np.zeros((self.dataShape[2],1))
            for n_vert_id in v.get_neighbors():
                interim_sum += interim_theta[0,indx+1]*np.multiply(self.reshaped_data[:,v.vertex_id-1], self.reshaped_data[:,n_vert_id-1]).reshape((-1,1))
            interim_sum += interim_theta[0,0]*self.reshaped_data[:,v.vertex_id-1].reshape((-1,1))
            interim_exp = np.exp(interim_sum)
            num_1 = np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)), interim_exp)
            den = np.add(1,interim_exp)
            ratio_1 = np.divide(num_1,den)
...
#            print "Gradient Edges 1 ", grad_1
            ...
            num_2 = np.multiply(np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)),self.reshaped_data[:,nb_vert_id-1].reshape((-1,1))),interim_exp)
            ratio_2 = np.divide(num_2,den)
            adj_ratio_2 = np.subtract(ratio_2,np.multiply(self.reshaped_data[:,v.vertex_id-1].reshape((-1,1)),self.reshaped_data[:,nb_vert_id-1].reshape((-1,1))))
    ...
    def forward_backward(self, v, step, epsilon, nu):
        while True:
            ...
            while True:
                ...
                v.reduce_set_edges()
                self.THETA[v.vertex_id-1,remove_edge_to_vert-1] = 0
                self.minimize_loss_theta(v, step)
#                np.copyto(self.THETA_previous, self.THETA)
...
#        interim_exp = self.minimize_loss_theta(self.vertices[0],0.01)
        for vertex in self.vertices[indices[0]:indices[1]]:
            theta = self.forward_backward(vertex,0.01,85.0,0.4)  
        print "###########################It Ended####################################"
        return self.THETA
...
np.savetxt("model_3_1.csv", results[0], delimiter=",")
