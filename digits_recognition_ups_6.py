...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 17 10:00:30 2018
...
#################################################################################################
    ...
    def train(self, indices):
        ...
        for vertex in self.vertices[indices[0]:indices[1]]:
            theta = self.forward_backward(vertex,0.001, 120.0,0.4)
        print("###########################It Ended ####################################")
        return self.THETA
...
data[data == 0] = -1
ising_model = Graph(data[3600:4200,:])
results = ising_model.train([0,256])
np.savetxt("model_ups_6.csv", results, delimiter=",")
