...
# -*- coding: utf-8 -*-
"""
Created on Tue Apr 10 10:16:24 2018
...
#        self.EDGE = np.zeros((self.dataShape[2],self.dataShape[2]))
        ...
        self.leftNodes = []
        for i in range(32,1024,31):
            self.leftNodes.append(i)
        self.rightNodes = []
        for i in range(63,1055,31):
            self.rightNodes.append(i)
        self.bottomNodes = []
        for i in range(993,1023):
        ...
        for vert in self.vertices:
            if vert.vertex_id == 0:
                vert.add_neighbor([1,32])
                vert.add_edges([0,0])
            elif vert.vertex_id == 31:
                vert.add_neighbor([30,63])
                vert.add_edges([0,0])
            elif vert.vertex_id == 992:
                vert.add_neighbor([960,993])
                vert.add_edges([0,0])
            elif vert.vertex_id == 1023:
                vert.add_neighbor([991,1022])
                vert.add_edges([0,0])
...
#        interim_exp = self.minimize_loss_theta(self.vertices[0],0.01)
        ...
        print "###########################It Ended####################################"
        np.savetxt("model_2.csv", self.THETA, delimiter=",")
...
ising_model = Graph(train_data[2]).train()
